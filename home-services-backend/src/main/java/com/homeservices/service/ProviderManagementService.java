...
public class ProviderManagementService {
    ...
    public List<ProviderServiceResponse> getMyServices(UUID userId) {
        Provider provider = getProviderByUserId(userId);
        return providerServiceRepository.findByProviderIdAndActiveTrue(provider.getId())
                .stream()
    ...
    @Transactional
    public BookingResponse updateBookingStatus(UUID userId, UUID bookingId, BookingStatusRequest request) {
        Provider provider = getProviderByUserId(userId);
        ...
        if (newStatus != BookingStatus.CONFIRMED
            ...
            throw new BadRequestException("Provider can only set status to CONFIRMED, REJECTED, or COMPLETED");
        }
